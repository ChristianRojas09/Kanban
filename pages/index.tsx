...
const Home: React.FC<HomeProps> = ({ prefetchedData = { boards: [] } }) => {
  ...
  return (
    <>
      ...
      <main className="text-black dark:text-white bg-[#F5EEE6] dark:bg-[#222831] h-screen flex border-solid">
        ...
        {openedTask && activeModal === ModalEnum.DELETE_TASK && (
          <Modal onClickBackdrop={() => dispatch(setOpenedTask(undefined))}>
            <Delete
              ...
              }’ task and its subtasks? This action is forever.`}
              onCancel={() => {
                dispatch(setActiveModal(undefined));
                dispatch(setOpenedTask(undefined));
              }}
...
export async function getServerSideProps() {
  ...
  //if there is no data in database read the data.json file
  if (data.length === 0) {
